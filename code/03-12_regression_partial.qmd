...
In this case boxplot is not the best way to understand this relationship. 
```{r reg exp boxplot}
ggplot(reg_dfw,
       aes(x = sr_ksha,
           y = yield_mgha)) +
...
## d) Final plot  
The '.fixed' column offere the regression line for this model. 
```{r mod1 final plot}
ggplot(mod1_int_aug, 
       aes(x = sr_ksha, 
           y = yield_mgha))+
...
# Each line will be the statistic of 200 models.   
  confint() %>%
  as.data.frame() %>%
...
```
Make sure what x and y values we are using for geom_ribbon. 
...
ggplot(mod2_lin_aug, 
  ...
  geom_smooth()
```
Apparently we are missing a quadratic term in this model. 
...
NOTICE: Column name needs to match column in the model
Level = 0 means give a prediction at a general level of the model. 
...
# 10) Extra - Bootstrapping
Estimate overall yield but with confidence levels. 

```{r}
library(infer)

boot <-reg_dfw %>%
  specify(response = yield_mgha) %>%
  generate(reps = 10000,
           type = 'bootstrap'
           )
